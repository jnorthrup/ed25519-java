...
() : Signature(SIGNATURE_ALGORITHM) {
    ...
    private var oneShotOffset = 0
    private var oneShotLength = 0
    ...
    override fun engineInitSign(privateKey: PrivateKey) {
        ...
        if (privateKey is EdDSAPrivateKey) {
            ...
            } else if (key!!.params.hashAlgorithm != digest!!.algorithm)
    ...
    private fun x_engineSign(): ByteArray {
        ...
        val a = (key as EdDSAPrivateKey).byteArr
        ...
        if (oneShotMode) {
            ...
            message = if (null == baos) BYTES else baos!!.toByteArray()
            offset = 0
            length = message.size
        }
        // r = H(h_b,...,h_2b-1,M)
     ...
     */
    @Throws(SignatureException::class)
    fun signOneShot(data: ByteArray): ByteArray {
        return signOneShot(data, 0, data.size)
    }
     ...
     */
    @Throws(SignatureException::class)
    fun signOneShot(data: ByteArray, off: Int, len: Int): ByteArray {
        oneShotMode = true
        update(data, off, len)
        return sign()
    }
    ...
    override fun engineSetParameter(spec: AlgorithmParameterSpec) {
        if (spec == ONE_SHOT_MODE) {
            if (null != oneShotBytes || null != baos && 0 < baos!!.size())
