...
class Ed25519LittleEndianEncoding : Encoding() {
    ...
    override fun decode(`in`: ByteArray): FieldElement {
        ...
        h[8] = h8.toInt()
        h[9] = h9.toInt()
        return Ed25519FieldElement(finiteField, h)
    }
    ...
    companion object {
        ...
        internal fun load_3(`in`: ByteArray, offset: Int): Int {
            var i = offset
            var result = `in`[i++] and 0xff
            result = result or (`in`[i++] and 0xff shl 8)
            result = result or (`in`[i] and 0xff shl 16)
            return result
        }
