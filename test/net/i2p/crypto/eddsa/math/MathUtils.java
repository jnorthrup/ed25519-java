...
public class MathUtils {
    ...
    public static GroupElement toGroupElement(final byte[] bytes) {
        ...
        bytes[31] &= 0x7f;
        final BigInteger y = toBigInteger(bytes);
        ...
        if (shouldBeNegative ? !isNegative : isNegative) {
            x = x.negate().mod(getQ());
        }
    ...
    public void mathUtilsWorkAsExpected() {
        ...
        for (int i = 0; 1000 > i; i++) {
            final GroupElement g1 = getRandomGroupElement();

            // P3 -> P2.
            final GroupElement h = toRepresentation(g1, GroupElement.Representation.P2);
            Assert.assertThat(h, IsEqual.equalTo(g1));
            // P3 -> P1P1.
            final GroupElement h2 = toRepresentation(g1, GroupElement.Representation.P1P1);
            Assert.assertThat(g1, IsEqual.equalTo(h2));

            // P3 -> CACHED.
            final GroupElement h2a = toRepresentation(g1, GroupElement.Representation.CACHED);
            Assert.assertThat(h2a, IsEqual.equalTo(g1));
       final    GroupElement g;
       final    GroupElement h4;
            {
                // P3 -> P2 -> P3.
                final GroupElement g2 = toRepresentation(g1, GroupElement.Representation.P2);
                {
                    final GroupElement h3 = toRepresentation(g2, GroupElement.Representation.P3);
        ...
        for (int i = 0; 10 > i; i++) {
            ...
            // Act:
            final GroupElement h = scalarMultiplyGroupElement(g, curve.getField().ZERO);
